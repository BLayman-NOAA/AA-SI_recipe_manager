...
def test_no_config_returns_empty():
    ...
    assert cfg.user_cache_dir is None
...
def test_explicit_path(tmp_path):
    p = _write(tmp_path / "c.yaml", {"user_cache_dir": "gs://b/cache"})
    cfg = config.load_run_config(str(p))
    assert cfg.source == p
    assert cfg.user_cache_dir == "gs://b/cache"
...
def test_explicit_beats_env(monkeypatch, tmp_path):
    envp = _write(tmp_path / "env.yaml", {"user_cache_dir": "env"})
    argp = _write(tmp_path / "arg.yaml", {"user_cache_dir": "arg"})
    monkeypatch.setenv(config.ENV_VAR, str(envp))
    assert config.load_run_config(str(argp)).user_cache_dir == "arg"


def test_autodiscovery_first_found_wins(monkeypatch, tmp_path):
    first = _write(tmp_path / "first.yaml", {"user_cache_dir": "first"})
    second = _write(tmp_path / "second.yaml", {"user_cache_dir": "second"})
    monkeypatch.setattr(
        config, "default_config_search_paths", lambda: [first, second]
    )
    assert config.load_run_config().user_cache_dir == "first"


def test_autodiscovery_skips_missing(monkeypatch, tmp_path):
    missing = tmp_path / "missing.yaml"
    present = _write(tmp_path / "present.yaml", {"user_cache_dir": "present"})
    monkeypatch.setattr(
        config, "default_config_search_paths", lambda: [missing, present]
    )
    assert config.load_run_config().user_cache_dir == "present"
...
def test_per_recipe_discovery(tmp_path):
    ...
    _write(tmp_path / "survey_a.config.yaml", {"user_cache_dir": "gs://a/cache"})

    cfg = config.load_run_config(recipe_path=recipe)
    assert cfg.user_cache_dir == "gs://a/cache"
    assert cfg.source == tmp_path / "survey_a.config.yaml"


def test_per_recipe_beats_generic_cwd(monkeypatch, tmp_path):
    generic = _write(tmp_path / "generic.yaml", {"user_cache_dir": "generic"})
    monkeypatch.setattr(config, "default_config_search_paths", lambda: [generic])

    recipe = tmp_path / "survey_b.yaml"
    recipe.write_text("recipe: {}\n", encoding="utf-8")
    _write(tmp_path / "survey_b.config.yaml", {"user_cache_dir": "per-recipe"})

    assert config.load_run_config(recipe_path=recipe).user_cache_dir == "per-recipe"
    # Without a per-recipe file, the generic fallback is used.
    other = tmp_path / "survey_c.yaml"
    other.write_text("recipe: {}\n", encoding="utf-8")
    assert config.load_run_config(recipe_path=other).user_cache_dir == "generic"
...
def test_env_var_beats_per_recipe(monkeypatch, tmp_path):
    ...
    recipe.write_text("recipe: {}\n", encoding="utf-8")
    _write(tmp_path / "survey_d.config.yaml", {"user_cache_dir": "per-recipe"})

    assert config.load_run_config(recipe_path=recipe).user_cache_dir == "env"
...
def test_two_recipes_same_dir_get_different_configs(tmp_path):
    for name, bucket in [("survey_a", "gs://a/cache"), ("survey_b", "gs://b/cache")]:
        ...
        _write(tmp_path / f"{name}.config.yaml", {"user_cache_dir": bucket})

    assert (
        config.load_run_config(recipe_path=tmp_path / "survey_a.yaml").user_cache_dir
        == "gs://a/cache"
    )
    assert (
        config.load_run_config(recipe_path=tmp_path / "survey_b.yaml").user_cache_dir
        == "gs://b/cache"
    )
...
def test_cwd_autodiscovery(monkeypatch, tmp_path):
    ...
    _write(tmp_path / config.CONFIG_FILENAME, {"user_cache_dir": "from-cwd"})
    assert config.load_run_config().user_cache_dir == "from-cwd"
...
def test_all_fields(tmp_path):
    ...
    cfg = config.load_run_config(str(p))
    assert cfg.user_cache_dir == "gs://b/c"
...
def test_bad_user_cache_dir_type_raises(tmp_path):
    p = _write(tmp_path / "c.yaml", {"user_cache_dir": ["a"]})
    with pytest.raises(ValueError, match="user_cache_dir"):
        config.load_run_config(str(p))
...
def test_cli_config_supplies_dirs_and_inputs(monkeypatch, tmp_path):
    ...
    assert captured["user_cache_dir"] == "gs://b/cache"
...
def test_cli_flags_override_config(monkeypatch, tmp_path):
    ...
    cfg = _write(
        tmp_path / "run.yaml",
        {"user_cache_dir": "gs://b/cache", "inputs": {"raw_input_folder": "gs://b/raw"}},
    )
    recipe = _dummy_recipe(tmp_path)
    ...
    assert result.exit_code == 0, result.output
    # CLI flag wins over config user_cache_dir; CLI --input wins over config input.
    assert captured["user_cache_dir"] == "./local_cache"
...
def test_cli_no_config_uses_builtin_default(monkeypatch, tmp_path):
    ...
    assert result.exit_code == 0, result.output
    assert captured["user_cache_dir"] == "./recipe_cache"
...
def test_cli_per_recipe_config_autodiscovered(monkeypatch, tmp_path):
    ...
    _write(tmp_path / "recipe.config.yaml", {"user_cache_dir": "gs://mine/cache"})

    result = CliRunner().invoke(cli.main, ["run", str(recipe)])
    assert result.exit_code == 0, result.output
    assert captured["user_cache_dir"] == "gs://mine/cache"
    assert "Using run config" in result.output
...
def test_survey_cache_dir_parsed_from_config(tmp_path):
    p = _write(
        tmp_path / "c.yaml",
        {"user_cache_dir": "gs://b/users/me/cache", "survey_cache_dir": "gs://b/surveys/HB1603/cache"},
    )
    cfg = config.load_run_config(str(p))
...
def test_cli_survey_cache_dir_from_config(monkeypatch, tmp_path):
    ...
    cfg = _write(
        tmp_path / "run.yaml",
        {"user_cache_dir": "gs://b/cache", "survey_cache_dir": "gs://b/surveys/X/cache"},
    )
    recipe = _dummy_recipe(tmp_path)
...
def test_cli_clean_honors_config(monkeypatch, tmp_path):
    """clean resolves user_cache_dir and storage_options from the run config."""
    captured: dict = {}

    def fake_clean(recipe, user_cache_dir, **kwargs):
        captured["user_cache_dir"] = user_cache_dir
        captured.update(kwargs)
        return []
    ...
    cfg = _write(
        tmp_path / "run.yaml",
        {"user_cache_dir": "gs://b/users/me/cache", "storage_options": {"token": "x"}},
    )
    recipe = _dummy_recipe(tmp_path)
    ...
    assert result.exit_code == 0, result.output
    assert captured["user_cache_dir"] == "gs://b/users/me/cache"
    assert captured["storage_options"] == {"token": "x"}
...
def test_cli_clean_flag_overrides_config(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(cli.api, "clean", fake_clean)
    cfg = _write(tmp_path / "run.yaml", {"user_cache_dir": "gs://b/users/me/cache"})
    recipe = _dummy_recipe(tmp_path)

    result = CliRunner().invoke(
        cli.main,
        ["clean", str(recipe), "--config", str(cfg), "--user-cache-dir", "./local"],
    )
    assert result.exit_code == 0, result.output
    assert captured["user_cache_dir"] == "./local"
